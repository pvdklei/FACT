...
class OpenAIClient:
    ...
    def __generate_group_of_cv_s_batch(self, cv_s_dataframe: pd.DataFrame):
        ...
        final_batch = self.client.batches.retrieve(self.batch_id)
        final_batch_status = final_batch.status
        if final_batch_status != 'completed':
            print(f"Final batch: {final_batch}")
            raise Exception(self.batch_id+" finished, but was: "+final_batch_status)
        
        else:
            #Ended.
            output_id = final_batch.output_file_id
